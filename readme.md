...
A specialized e-commerce platform for dental college students to order dental instruments and products, built on top of Bagisto.

## Features

- Custom registration with college selection
- OTP verification via SMS and Email
- College-based delivery system
- Admin panel for order management
- Product categories specific to dental needs
...
### SMS Gateway
Configure your SMS gateway (Fast2SMS or Msg91) in `.env`:
```
...
### Custom Modules
The application uses two custom modules:
- `CollegeModule`: Handles college selection and management
- `OTPModule`: Manages OTP generation and verification

### Adding New Colleges
To add new colleges, use the seeder:
```bash
php artisan db:seed --class=Dental\\CollegeModule\\Database\\Seeders\\CollegeSeeder
```
